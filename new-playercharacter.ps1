<#
    ...
    1. Creates an entity entry under ## Postać (Gracz) with @należy_do and
...
function New-PlayerCharacter {
    ...
    $Section = Find-EntitySection -Lines $File.Lines.ToArray() -EntityType 'Postać (Gracz)'
    if ($Section) {
    ...
    # Create character entry under ## Postać (Gracz)
    $InitialTags = [ordered]@{
        'należy_do'  = $PlayerName
        'pu_startowe' = $PUStartStr
    }

    $CharTarget = Resolve-EntityTarget -FilePath $EntitiesFile -EntityType 'Postać (Gracz)' -EntityName $CharacterName -InitialTags $InitialTags
