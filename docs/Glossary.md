...
|---|---|---|
| Active Character | Aktywna postać | The player character currently treated as a player's main character. |
...
| Entity | Encja (element świata) | Any named world element tracked by the system, such as a character, location, group, map, player, or item. |
| Entity Store | Rejestr encji | The central registry that holds base properties and history for tracked world entities. |
| Entity Type | Typ encji | The category of an entity: Player (Gracz), Character (Postać), Location (Lokacja), Map (Mapa), Group (Grupa), or Item (Przedmiot). |
...
| Soft Delete | Miękkie usunięcie | Marking something as removed for future queries without erasing its history. |
...
| Session Header | Nagłówek sesji | The unique session title line in the form `### YYYY-MM-DD, Title, Narrator`. |
...
| Quantity (`@ilość`) | `@ilość` | The quantity field used for stackable items such as currency. |
...
PU (Punkty Umiejętności) are skill points assigned to characters through campaign play. A monthly assignment run calculates how much each character earns and records it in the processing history.
...
| Monthly Cap | Miesięczny limit PU | The maximum PU a character can receive in one month before overflow rules apply. |
| Monthly PU Assignment | Miesięczny przydział PU | The regular process that calculates and grants PU for the selected period. |
...
| PU (Skill Points) | Punkty Umiejętności (PU) | Skill points assigned to characters as part of campaign progression. |
| Starting PU | PU startowe | The baseline PU a character begins with. |
| Total PU | PU suma | The character's overall PU total. |
| Voting Eligibility | Uprawnienia do głosowania | Whether a player qualifies to vote based on recent PU history. |
...
| Currency Denomination | Nominał waluty | One of the three money types: Korona, Talar, or Kog. |
...
| Narrator Budget | Budżet narratora | Currency distributed to a narrator before play so it can later be awarded in sessions. |
| Talar (Silver) | Talar Hiroński | The mid-value currency denomination. 1 Talar = 100 Kogów. |
...
Cross-cutting features that support data quality, search, and audit.

| English term | Polish counterpart | Meaning in plain language |
|---|---|---|
| Change Log | Log zmian | A chronological report of world-state changes recorded across sessions. |
| Fuzzy Search | Wyszukiwanie rozmyte | A picker that lets you find a record by partial or approximate spelling. |
| Name Resolution | Rozpoznawanie nazw | The system's matching of names in sessions, reports, and notifications to known entities. |
| Unresolved Name | Nierozpoznana nazwa | A name the system could not match to a known entity, which may block or weaken processing. |
