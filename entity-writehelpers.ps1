<#
    ...
    - Ensure-EntityFile:          ensures entities.md exists with required sections
...
function Resolve-EntityTarget {
    ...
    $FilePath = Ensure-EntityFile -Path $FilePath
    ...
    if (-not $Bullet) {
        $NewEnd = New-EntityBullet -Lines $Lines -SectionEnd $Section.EndIdx -EntityName $EntityName -Tags $InitialTags
        $Created = $true
...
function ConvertTo-EntitiesFromPlayers {
    ...
    # Postać (Gracz) section
    [void]$SB.Append("`n")
    [void]$SB.Append("## Postać (Gracz)")
    [void]$SB.Append("`n")
