...
`Invoke-PlayerCharacterPUAssignment` is the primary pipeline function. Computes and optionally applies monthly PU awards.
...
`Test-PlayerCharacterPUAssignment` is a diagnostic validation wrapper. Runs the pipeline in compute-only mode (`-WhatIf`) and validates data quality.
...
Parameters: `Year`, `Month`, `MinDate`, `MaxDate` (same semantics as `Invoke-PlayerCharacterPUAssignment`, but no side-effect switches), plus `ExcludeDirectory` (string[]) and `Quiet` (switch).

`Get-NewPlayerCharacterPUCount` is a pure computation function for new character starting PU.
...
Diagnostic default range: `Test-PlayerCharacterPUAssignment` uses a slightly different default when no Year/Month or explicit dates are provided — MinDate is 1st of the previous month and MaxDate is tomorrow (`Now.AddDays(1)`). This extends the window forward to catch today's sessions.
...
If any PU entry references a character name that cannot be resolved, the pipeline throws a terminating error (`ErrorId: UnresolvedPUCharacters`, `ErrorCategory: InvalidData`) and aborts all processing. No PU is awarded, no Discord messages are sent, no log entries are appended.
...
The `+1` is an unconditional universal monthly base granted to any character that appears in PU entries. See the Divergences from Rules Document section for details.

Overflow pool interaction:
...
Decimal PU values are granted as-is (e.g., 3.70 PU is granted as 3.70). See the Divergences from Rules Document section. The overflow pool (`PUExceeded`) is unbounded.

Updated character totals:
...
Worked example — Character Crag Hack has: `PUSum = 45.00`, `PUTaken = 25.00`, `PUStart = 20.00`, `PUExceeded = 1.50`. Sessions in the batch award Crag Hack: 0.3 + 0.5 + 0.2 = 1.0 PU.
...
Overflow example — Character Gem has: `PUSum = 80.00`, `PUTaken = 60.00`, `PUStart = 20.00`, `PUExceeded = 0.00`. Sessions award Gem: 2.0 + 1.5 + 1.0 + 0.5 = 5.0 PU.
...
UpdatePlayerCharacters writes updated values to `entities.md` via `Set-PlayerCharacter`. Three fields are written per character:
...
`Set-PlayerCharacter` resolves the entity target in `entities.md` under `## Postac`, creating the entry with `@nalezy_do: <PlayerName>` if it does not exist. PU derivation rules (`Complete-PUData`) may auto-derive missing fields (e.g., if SUMA is given but ZDOBYTE is missing, ZDOBYTE = SUMA - STARTOWE).

PUTaken is written explicitly to ensure data consistency.

SendToDiscord groups notifications per player. All characters belonging to the same player are combined into a single message, sent via the player's `PRFWebhook`.

Grouping uses a `Dictionary[string, List[object]]` (OrdinalIgnoreCase) keyed by `PlayerName`. Characters without a `PlayerName` are skipped.

Message format (Polish, mandatory) — for each character in the player's results:
```
...
- PU entries for non-matching players' characters are silently skipped.
...
## Divergences from Rules Document

The rules document (`Tworzenie i Rozwoj Postaci.md`) states:

> "Postac otrzymuje tylko pelne zgromadzone PU i nie otrzymuje nadwyzki"
...
This specification grants decimal PU values as-is. The overflow pool (`PUExceeded`) accumulates only the excess beyond the 5 PU monthly cap. This is a deliberate implementation decision reflecting long-standing practice. Both the legacy and current implementations have always granted decimal PU.
...
IncludedCharacters = all characters of this player where PUStart > 0
                     AND status != Usuniety (when Entities are provided)
