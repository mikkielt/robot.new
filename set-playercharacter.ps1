<#
    ...
    Target 1 — entities.md:
      ...
      under ## Postać (Gracz). Auto-creates Przedmiot entities for unknown items.
...
. "$PSScriptRoot/charfile-helpers.ps1"
...
function Set-PlayerCharacter {
    ...
    if ($Target.Created -eq $false) {
        ...
        if (-not $OwnerTag) {
            $ChildEnd = Set-EntityTag -Lines $Lines -BulletIdx $Target.BulletIdx -ChildrenStart $Target.ChildrenStart -ChildrenEnd $ChildEnd -TagName 'należy_do' -Value $PlayerName
        }
    }
    ...
    if ($PSBoundParameters.ContainsKey('PUStart') -and $null -ne $DerivedPUStart) {
        ...
        $ChildEnd = Set-EntityTag -Lines $Lines -BulletIdx $Target.BulletIdx -ChildrenStart $Target.ChildrenStart -ChildrenEnd $ChildEnd -TagName 'pu_startowe' -Value $Val
    }
    ...
    if ($PSBoundParameters.ContainsKey('PUExceeded') -and $null -ne $PUExceeded) {
        ...
        $ChildEnd = Set-EntityTag -Lines $Lines -BulletIdx $Target.BulletIdx -ChildrenStart $Target.ChildrenStart -ChildrenEnd $ChildEnd -TagName 'pu_nadmiar' -Value $Val
    }
    ...
    if (($PSBoundParameters.ContainsKey('PUSum') -or $null -ne $DerivedPUSum) -and $null -ne $DerivedPUSum) {
        ...
        $ChildEnd = Set-EntityTag -Lines $Lines -BulletIdx $Target.BulletIdx -ChildrenStart $Target.ChildrenStart -ChildrenEnd $ChildEnd -TagName 'pu_suma' -Value $Val
    }
    ...
    if (($PSBoundParameters.ContainsKey('PUTaken') -or $null -ne $DerivedPUTaken) -and $null -ne $DerivedPUTaken) {
        ...
        $ChildEnd = Set-EntityTag -Lines $Lines -BulletIdx $Target.BulletIdx -ChildrenStart $Target.ChildrenStart -ChildrenEnd $ChildEnd -TagName 'pu_zdobyte' -Value $Val
    }
    ...
    if ($PSBoundParameters.ContainsKey('Status')) {
        ...
        $ChildEnd = Set-EntityTag -Lines $Lines -BulletIdx $Target.BulletIdx -ChildrenStart $Target.ChildrenStart -ChildrenEnd $ChildEnd -TagName 'status' -Value "$Status ($DateStr`:)"
    }
